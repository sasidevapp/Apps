...
<%@page import="AllLayout.*,com.sasi.*,java.util.*" %>
<%
	String message = "";
...
Buses trn = new Buses();
ArrayList<Bus> trainlist = new ArrayList<Bus>();
trainlist = trn.getAll();
Iterator trnIt = trainlist.iterator();


Destination desti = new Destination();
List<HashMap<String,String>> allDestinaions = new ArrayList<HashMap<String,String>>();
List<Object> stationList = sts.getAll();
...
if(request.getParameter("save_all") != null){
	...
	for(int j = 0; j<station_toAr.length; j++){
		...
		tempDesti.busId= request.getParameter("dst_train");
	...
	<% }else{ %>
		<div class="rs_box" style="overflow:auto;">
			<form class="ticket_selecting_form" method="post" name="frm_deslist">
				<h2 class="title"> Bus Name: CITY <br>Station From: Chennai </h2>
				<table class="table table_des_seat" id="table_des_list">
					...
					while(itrTemp.hasNext()){
						...
						Buses bus = new Buses(tempDestination.get("bus_id"));
						%>
						<tr>
							<td>
								<div class="input-group">
